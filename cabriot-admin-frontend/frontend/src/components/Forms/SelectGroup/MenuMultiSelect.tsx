...
interface MultiSelectProps {
    ...
    onKitchenChange: (cities: number[]) => void;
}

const MenuMultiSelect: React.FC<MultiSelectProps> = ({ id, selectedCities, onKitchenChange }) => {
    ...
    const select = (index: number, event: React.MouseEvent) => {
        ...
        const updatedCitiesIndex = newOptions.filter((option) => option.selected).map((option) => option.index);
        // console.log('kitchenIndex',updatedCitiesIndex)
        onKitchenChange(updatedCitiesIndex); // Notify parent component about the change
    };
    ...
    const remove = (index: number) => {
        ...
        if (selectedIndex !== -1) {
            ...
            setSelectOptions(newOptions);
            const updatedCities = newOptions.filter((option) => option.selected).map((option) => option.value);
            onKitchenChange(updatedCities); // Notify parent component about the change
        }
    };

    const selectedValues = () => {
        return selected.map((option) => selectOptions[option].value);
    };
                                    ...
                                    <div className="flex flex-auto flex-wrap gap-3">
                                        ...
                                        {selected.length === 0 && (
                                            <div className="flex-1">
                                                <input
                                                    placeholder="Select one or more kitchens"
