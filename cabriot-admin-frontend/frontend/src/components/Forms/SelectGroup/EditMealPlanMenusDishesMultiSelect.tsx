...
const EditMealPlanMenusDishesMultiSelect: React.FC<MultiSelectProps> = ({ id, options,  dishesData,  selectedDishes, selectedMeals, onChange, checkboxDisabled, checkPriceCheckbox }) => {
    ...
    useEffect(() => {
        ...
        const newOptions: Option[] = options.map((option, index) => ({
            ...
            text:  option.name,
            category: "meal",
            selected: selectedMeals && selectedMeals.length > 0 && selectedMeals.some(city => city.name === option.name),
        }));
        const newOptions1: Option[] = dishesData.map((option, index) => ({
            value: option.id,
            text: option.name,
            category: "dish",
            // selected: selectedDishes.some(city => city.name === option.name),
            selected: selectedDishes && selectedDishes.length > 0 && selectedDishes.some(city => city.name === option.name),
        ...
        // console.log('selected length', initiallySelectedIndices.length);

        const initiallySelectedIndicesd = newOptions1
            .map((option, index) => (option.selected ? index + selected.length : -1))
            .filter((index) => index !== -1);
        setSelected([...selected, ...initiallySelectedIndicesd]);
        ...
        loadOptions();
    }, [options, selectedDishes, selectedMeals ]);
...
export default EditMealPlanMenusDishesMultiSelect;
