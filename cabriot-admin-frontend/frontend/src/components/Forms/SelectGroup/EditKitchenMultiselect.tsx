...
interface MultiSelectProps {
  ...
  onKitchenChange: (cities: string[]) => void;
}

const EditKitchenMultiselect: React.FC<MultiSelectProps> = ({ id, options, selectedCities, onKitchenChange }) => {
  const [selected, setSelected] = useState<number[]>(selectedCities.map(city => options.indexOf(city)));
  const [show, setShow] = useState(false);
  const [selectOptions, setSelectOptions] = useState<Option[]>([]); // Manage options with state
  ...
  useEffect(() => {
    ...
    const loadOptions = () => {
      const newOptions: Option[] = options.map((option, index) => ({
        value: option,
        text: option,
        selected: selectedCities.includes(option), // Set selected based on selectedCities prop
      }));
...
export default EditKitchenMultiselect;
