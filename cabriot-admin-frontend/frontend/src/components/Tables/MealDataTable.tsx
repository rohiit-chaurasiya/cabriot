...
import EditMeal from '../../Forms/EditMeal';
...
const MealDataTable: React.FC<EditFormProps> = ({handleEditForm}) => {
    ...
    const fetchDishes = async () => {
        try {
            const response = await axios.get(
                "https://admindashboard-fr4p.onrender.com/api/mealinfo"
            );
            const data = response.data;
            setDishes(data);
            // console.log('Dishes  Info data :', data);
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`${baseUrl}/api/mealinfo/${id}`);
                        ...
                        <tbody>{currentDishes.map((packageItem, key) => (
                            ...
                            <tr className="border-b border-[#eee] dark:border-strokedark" key={key}>
                                <td className="border-[#eee] py-5 px-4 pt-6 dark:border-strokedark flex items-center">
                                    <img src={packageItem.MealImgUrl} className="h-12 w-12 mr-2 rounded-full" alt="User" />
                                    <span>{packageItem.meal_name}</span>
                                </td>
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark ">
                                    {parseFloat(packageItem.meal_base_price).toFixed(2)}
                                </td>
                                
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    {(packageItem.meal_weight)}
                                </td>


                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    {packageItem.dietary_choices == 1
                                        ? "Veg"
                                        : packageItem.dietary_choices == 2
                                            ? "Non Veg"
                                            : packageItem.dietary_choices == 3
                                                ? "Vegan"
                                                : "Not Defined"}
                                </td>

                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark ">
                                    {packageItem.cuisine_choices && packageItem.cuisine_choices.length > 0 ? (
                                        <p>
                                            {packageItem.cuisine_choices.map((cuisineId, index) => (
                                                <span key={index}>
                                                    {cuisineData[cuisineId] || 'Unknown Cuisine'}
                                                    {index < packageItem.cuisine_choices.length - 1 && ", "}
                                                </span>
                                            ))}
                                        </p>
                                    ) : (
                                ...
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    {packageItem.meal_choices && packageItem.meal_choices.length > 0 ? (
                                        <p>
                                            {packageItem.meal_choices.map((mealId, index) => (
                                                <span key={index}>
                                                    {mealData[mealId] || 'Unknown Meal'}
                                                    {index < packageItem.meal_choices.length - 1 && ", "}
                                                </span>
                                            ))}
                                        </p>
                                    ) : (
                                ...
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    {packageItem.selected_dishes && packageItem.selected_dishes.length > 0 ? (
                                        <p>
                                            {packageItem.selected_dishes.map((cuisineId, index) => (
                                                <span key={index}>
                                                    {`${DishData[cuisineId] || 'Unknown Cuisine'} - ${packageItem.meal_qty[index] ? packageItem.meal_qty[index][1] : 'Quantity Not Available'}`}
                                                    {index < packageItem.selected_dishes.length - 1 && ", "}
                                                </span>
                                            ))}
                                        </p>
                                    ) : (
                                ...
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <p
                                        className={`inline-flex rounded-full capitalize bg-opacity-10 py-1 px-3 text-sm font-medium ${packageItem.meal_availability_status === 'available'
                                            ? 'bg-success text-success' // Show green for available
                                            : 'bg-danger text-danger'    // Show red for anything else (assuming 'unavailable')
                                            }`}
                                    >
                                        {packageItem.meal_availability_status}
                                    </p>
                                ...
                                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    <div className="flex items-center space-x-3.5">
                                        ...
                                        <button className="hover:text-primary" onClick={() => handleDelete(packageItem.meal_id)}>
                                            <svg
            ...
            {show && 
                <>
                    <div className="flex justify-between text-lg p-3 mb-5 container bg-white rounded-lg">
                        <div>
                        <h1 className="font-semibold text-slate-950 dark:text-white p-2">Update Meal Details</h1>
                        </div>
