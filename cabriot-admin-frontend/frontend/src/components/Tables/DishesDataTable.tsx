...
import EditDish from '../../Forms/EditDish';
...
const DishesDataTable: React.FC<EditFormProps> = ({handleEditForm}) => {
    ...
    const fetchCuisineData = async () => {
        try {
            ...
            const data = response.data;
            // console.log('Cuisine data:', data); // Log the fetched cuisine data
            const cuisineObj = {};
    ...
    const fetchMealsData = async () => {
        try {
            ...
            const data = response.data;
            // console.log('Meal data:', data); // Log the fetched meal data
            const mealObj = {};
            data.forEach(meal => {
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`${baseUrl}/api/dishinfo/${id}`);
    ...
    return (<>
            ...
            <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
                <div className="max-w-full overflow-x-auto">
                    <table className="w-full table-auto">
                        ...
                        <tbody>
                            {currentDishes.map((packageItem, key) => (
                                <tr className="border-b border-[#eee] dark:border-strokedark" key={key} >
                                    <td  className="border-[#eee] py-5 px-4 pl-0 dark:border-strokedark xl:pl-3 flex items-center">
                                        <img src={packageItem.DishImgUrl} className="h-12 w-12 mr-2 rounded-full" alt="User" />
                                        <span>{packageItem.dish_name}</span>
                                    </td>
                                    ...
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">
                                            {parseFloat(packageItem.dish_weight).toFixed(2)} GM
                                        </p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                    ...
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <div className="flex items-center space-x-3.5">
                                            <button
                                                ...
                                                // onClick={() => {handleEditFormChild (packageItem.dishId) }}
                                                onClick={
                                                    () => {
                                                        Setshow(true);
        ...
        {show && 
            <>
                {/* <h1 className='flex justify-end p-3 text-xl' onClick={() => { Setshow(false); fetchDishes(); }}> Back </h1> */}
                <div className="flex justify-between text-lg p-3 mb-5 container bg-white rounded-lg">
                    <div>
                    <h1 className="font-semibold text-slate-950 dark:text-white p-2">Update Dish Details</h1>
                    </div>
